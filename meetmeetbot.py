...
# gloval variables
users_keys = ['id', 'name', 'sex', 'age', 'city', 'description', 'photo', 'interests', 'mode_text', 'mode_num',
              'profile_created', 'username', 'chat_id', 'report_status_self']
explore_settings_keys = ['id', 'chat_id', 'username', 'likes_got_counter', 'account_status', 'activity_status',
                         'longtitude_self', 'latitude_self', 'report_status_self', 'reported_times',
                         'times_my_profile_shown', 'age_min', 'age_max', 'sex_required', 'interest_required',
                         'city_required', 'id_current', 'likes_to', 'dislikes_to', 'query', 'seen_profiles',
                         'query_length']
basic_interests = ['build', 'tik-tok', 'games', 'chess', 'work', 'party', 'languages', 'art', 'business', 'pets',
                   'anime', 'coding', 'travel', 'chatting', 'sea', 'music', 'photo', 'concerts', 'trading', 'auto',
                   'vacation', 'design', 'sport', 'literature', 'youtube', 'marketing', 'economics', 'videoblog',
                   'dance', 'startup', 'theater', 'children']
editable_settings = ['Name', 'Age', 'City', 'Sex', 'Description', 'Interests', 'Photo']
reactions = [emoji.emojize(':red_heart:'), emoji.emojize(':heart_with_arrow:'), emoji.emojize(':thumbs_down:'),
             emoji.emojize(':stop_sign:')]
...
def send_my_profile(chat_id, user):
    ...
    bot.send_photo(chat_id, open(f"images/{'fe' * int(user['sex'] != 'M')}male/{user['id']}.jpg", 'rb'),
...
def send_profile_explore(chat_id, user):
    caption_generated = f"{user['name']}, {user['age']}, {user['city']}\n\n{user['description']}\n\nInterests: {', '.join(user['interests'].split()) + '.'}"
    bot.send_photo(chat_id, open(f"images/{'fe' * int(user['sex'] != 'M')}male/{user['id']}.jpg", 'rb'),
                   caption=caption_generated, reply_markup=markup.explore)
...
def get_explore_settings(cursor, user_id):
    ...
    return dict(
        zip(explore_settings_keys, cursor.execute(f"SELECT * FROM explore_settings WHERE id == {user_id}").fetchone()))
...
def create_user(cursor, message):
    cursor.execute(
        f"INSERT INTO users VALUES ({message.from_user.id},'','','','','','','','main_menu',0,0,'{message.from_user.username}',{message.chat.id}, 'not_reported')")
    cursor.execute(
        f"INSERT INTO explore_settings VALUES ({message.from_user.id}, {message.chat.id}, '{message.from_user.username}', 0, 'free', 'active', 0, 0, 'not_reported', 0, 0, 0, 0, '', '', '', -1, '[]', '[]', '[]', '[]', 0)")
...
def create_query(user_id, cursor, settings, user):  # with filters
    ...
    new_query = cursor.execute(
        f"SELECT id FROM users WHERE id != {user_id} AND sex != '{user['sex']}' AND id NOT IN {seen_str} AND profile_created == 1").fetchall()
    random.shuffle(new_query)
    cursor.execute(
        f"UPDATE explore_settings SET query = '{ats(new_query)}', query_length = {len(new_query)} WHERE id = {user_id}")
...
def next_profile(cursor, user_id):
    ...
    cursor.execute(
        f"UPDATE explore_settings SET id_current = {anket_arr[0]}, query = '{ats(anket_arr[1:])}', query_length = {len(anket_arr) - 1} WHERE id == {user_id}")
...
def like(cursor, user, settings):
    ...
    previous = cursor.execute(
        f"SELECT likes_got_counter FROM explore_settings WHERE id == {settings['id_current']}").fetchone()
    cursor.execute(
        f"UPDATE explore_settings SET likes_got_counter = {previous + 1} WHERE id == {settings['id_current']}")
...
def profile_seen(cursor, explorer_id, explored_id):
    ...
    cursor.execute(
        f"UPDATE explore_settings SET seen_profiles = '{ats([explored_id] + seen)}' WHERE id == {explorer_id}")
...
def dislike(cursor, explorer_id, explored_id):
    ...
    cursor.execute(
        f"UPDATE explore_settings SET seen_profiles = '{ats([explorer_id] + seen)}' WHERE id == {explored_id}")
    ...
    if explorer_id in query:
        cursor.execute(
            f"UPDATE explore_settings SET query = '{ats(query)}', query_length = {len(query) - 1} WHERE id == {explored_id}")
    else:
        pass


def report(cursor, explorer_id, explored_id):
    reported_times_old = cursor.execute(
        f"SELECT reported_times FROM explore_settings WHERE id == {explored_id}").fetchone()
    times_my_profile_shown_old = cursor.execute(
        f"SELECT times_my_profile_shown FROM explore_settings WHERE id == {explored_id}").fetchone()
    ...
    try:
        ...
        if message.chat.type == 'private' and message.from_user.id in list(cursor.execute("SELECT id FROM users")):
            ...
            if (settings['report_status_self'] == 'reported'):
                bot.send_message(user_id,
                                 'You are reported so many times that we have blocked your account. Unblock via paying')
                pass
                ...
                text_var = re.sub("'", '', message.text)
                if user['mode_num'] == 1 and message.text != 'Back to main menu':
                    cursor.execute(f"UPDATE users SET name = '{text_var}', mode_num = 2 WHERE id = {user_id};")  # name
                    bot.send_message(message.chat.id, 'Male / Female?', reply_markup=markup.sex_create)
                elif user['mode_num'] == 2 and (message.text.find("Male") != -1 or text_var.find(
                        "Female") != -1) and message.text != 'Back to main menu':
                    ...
                    cursor.execute(f"UPDATE users SET city = '{text_var}', mode_num = 4 WHERE id = {user_id};")  # city
                    ...
                    cursor.execute(f"UPDATE users SET age = '{text_var}', mode_num = 5 WHERE id = {user_id};")  # age
                    bot.send_message(message.chat.id, emoji.emojize(
                        'Choose your interest. To end press continue:fast-forward_button: button at the bottom.'),
                                     reply_markup=create_interests(user['interests']))
                elif user['mode_num'] == 5 and ('continue' not in message.text) and message.text in (
                        set(basic_interests) - set(user['interests'].split())) and message.text != 'Back to main menu':
                    cursor.execute(
                        f"UPDATE users SET interests = '{user['interests'] + ' ' + message.text}', mode_num = 5 WHERE id = {user_id};")  # append interests 1 by 1
                    user = get_profile(cursor, user_id)
                    bot.send_message(message.chat.id, emoji.emojize(
                        'Added! Anything else? To end press continue:fast-forward_button: button at the bottom.'),
                                     reply_markup=create_interests(user['interests']))
                    ...
                    bot.send_message(message.chat.id, 'Write something about yourself here',
                                     reply_markup=markup.back_to_settings)
                elif user['mode_num'] == 6 and message.text != 'Back to main menu':
                    cursor.execute('UPDATE users SET description = ?, mode_num = 7 WHERE id = ?',
                                   (message.text, user_id))  # discription of profile
                    bot.send_message(message.chat.id, 'Send your photo for others to see your appearence',
                                     reply_markup=markup.back_to_settings)
                else:
                    cursor.execute(f"UPDATE users SET mode_num = 0, mode_text = 'main_menu' WHERE id = {user_id}")
                    cursor.execute(
                        f"UPDATE users SET name = '', sex = '', age = 0, city = '', description = '', interests = '' WHERE id = {user_id}")
                    bot.send_message(message.chat.id, 'Returning you to main menu. Your profile data was cleared.',
                                     reply_markup=markup.main)
                ...
                if message.text == 'Interests':
                    ...
                    bot.send_message(message.chat.id, emoji.emojize(
                        'Choose your interest. To end press continue:fast-forward_button: button at the bottom.'),
                                     reply_markup=create_interests(''))
                elif message.text == 'Sex':
                    bot.send_message(message.chat.id, f"Enter the new {message.text.lower()}",
                                     reply_markup=markup.sex_edit)
                    ...
                    bot.send_message(message.chat.id, f"Enter the new {message.text.lower()}",
                                     reply_markup=markup.back_to_settings)
                cursor.execute(
                    f"UPDATE users SET mode_num = {editable_settings.index(message.text) + 1} WHERE id = {user_id}")

            elif user['mode_text'] == 'profile_editing' and user[
                'mode_num'] != 0 and message.text != 'Back to main menu':

                if user['mode_num'] == editable_settings.index('Interests') + 1:
                    if ('continue' not in message.text) and message.text in (
                            set(basic_interests) - set(user['interests'].split())):
                        cursor.execute(
                            f"UPDATE users SET interests = '{user['interests'] + ' ' + message.text}' WHERE id = {user_id};")  # append interests 1 by 1
                        user = get_profile(cursor, user_id)
                        bot.send_message(message.chat.id, emoji.emojize(
                            'Added! Anything else? To end press continue:fast-forward_button: button at the bottom.'),
                                         reply_markup=create_interests(user['interests']))
                    elif user['mode_num'] == editable_settings.index(
                            'Interests') + 1 and 'continue' in message.text and message.text != 'Back to main menu':
                        bot.send_message(message.chat.id, 'Your interests field is updated. This is your profile now.',
                                         reply_markup=markup.edit)
                    ...
                    if user['mode_num'] == editable_settings.index('Sex') + 1:
                        if 'Male' in message.text or 'Female' in message.text:
                            cursor.execute(
                                f"UPDATE users SET sex = '{'M' if 'Male' in message.text else 'F'}', photo = 'images/{'male' if 'Male' in message.text else 'female'}/{user_id}.jpg' WHERE id = {user_id}")
                            os.rename(f"images/{'male' if user['sex'] == 'M' else 'female'}/{user_id}.jpg",
                                      f"images/{'male' if 'Male' in message.text else 'female'}/{user_id}.jpg")
                        else:
                        ...
                        text_var = re.sub("'", '', message.text)
                        cursor.execute(
                            f"UPDATE users SET {editable_settings[user['mode_num'] - 1].lower()} = '{text_var}' WHERE id = {user_id}")

                    cursor.execute(f"UPDATE users SET mode_num = 0 WHERE id = {user_id}")
                    bot.send_message(message.chat.id,
                                     f"Your {editable_settings[user['mode_num'] - 1].lower()} field is updated. This is your profile now.",
                                     reply_markup=markup.edit)
                    user = get_profile(cursor, user_id)
                    send_my_profile(message.chat.id, user)
                ...
                if (settings['account_status'] == 'vip'):
                    bot.send_message(message.chat.id, 'Explore page',
                                     reply_markup=vip_markup(settings['account_status']))
                else:
                    bot.send_message(message.chat.id, 'Explore page',
                                     reply_markup=vip_markup(settings['account_status']))
                text_var = ':red_heart: - Send your profile and tag to the user with heart directly.\n\n:heart_with_arrow: - Anonymous sympathy, we will send each of you profiles and tags if this is mutual.\n\n:thumbs_down: - You dislike the profile. Yours will not be shown to that person.\n\n:stop_sign: - Report user for bad behaviour.'
                bot.send_message(message.chat.id, 'Here the instruction:',
                                 reply_markup=vip_markup(settings['account_status']))
                bot.send_message(message.chat.id, emoji.emojize(text_var))
                cursor.execute(f"UPDATE users SET mode_text = 'explore_menu' WHERE id = {user_id}")
            ...
            elif message.text == 'Back to menu' and (
                    user['mode_text'] == 'explore' or user['mode_text'] == 'explore_settings'):
            ...
            create_user(cursor, message)
            bot.send_message(message.chat.id, 'Hello! Start chatting with creating your profile in settings.',
                             reply_markup=markup.main)
...
def reply_to_photo(message):
    ...
    if user['mode_num'] == 7 and user['mode_text'] == 'creating_profile':
        ...
        cursor.execute(
            f"UPDATE users SET mode_num = 0, mode_text = 'main_menu', photo = 'images/{'fe' * int(user['sex'] != 'M')}male/{user_id}.jpg', profile_created = 1 WHERE id = {user_id}")
        bot.send_message(message.chat.id,
                         'The creation of your profile is done. Now you can explore other profiles. This is your profile now',
                         reply_markup=markup.edit)
        send_my_profile(message.chat.id, user)
        bot.send_message(message.chat.id, 'You can edit your profile in profile settings.', reply_markup=markup.edit)
